-- Ermittelt für send-notifications, welche Push-Abos JETZT was bekommen sollen –
-- komplett in der Datenbank statt (wie bis 2026-09-25) alle Abos/Felder/Einstellungen
-- aller Nutzer bei jedem 15-Minuten-Lauf in die Edge Function zu laden und dort zu
-- filtern. Liefert nur Zeilen für Abos, bei denen tatsächlich etwas fällig ist.
--
-- Pro Nutzer in dessen eigener Zeitzone (user_settings.timezone, folgt dem Gerät,
-- siehe logbuch.html syncTimezone): lokales Datum ("heute") und lokaler
-- Viertelstunden-Slot (abgerundet – ein um ein paar Minuten verspäteter Cron-Lauf
-- verpasst so keine Erinnerung). Fällig ist:
-- - default_reminder: Slot = Standard-Erinnerungszeit und mindestens ein aktives Feld
--   ohne eigene Zeit ist heute noch leer.
-- - custom_missing: Namen der heute noch leeren Felder, deren eigene Zeit dieser Slot ist.
-- - week_summary / month_summary: Slot = Standardzeit, Übersichten nicht abgeschaltet,
--   Sonntag bzw. Monatsletzter, und im Zeitraum mind. ein Tag mit Eintrag.
-- Gruppen (kind='group') sind nie direkt befüllbar, tauchen nie in filled_slugs auf –
-- ohne Ausschluss würden sie dauerhaft als "fehlend" gelten.
--
-- Liefert Push-Endpoints/-Schlüssel ALLER Nutzer – deshalb nur für service_role
-- ausführbar (siehe REVOKE/GRANT unten), nie für anon/authenticated.
CREATE OR REPLACE FUNCTION public.get_due_notifications()
  ...
  with local_time as (
    select s.user_id, s.locale, s.summary_notifications, s.default_reminder_minute,
           now() at time zone s.timezone as ts
...
REVOKE ALL ON FUNCTION public.get_due_notifications() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_due_notifications() TO service_role;

-- Für den exists-/join-Lookup oben (bisher kein Index auf user_id, nur auf endpoint).
